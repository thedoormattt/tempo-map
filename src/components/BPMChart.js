...
export default function BPMChart({ curve, duration, playhead, avgBPM }) {
  ...
  const yTicks = [];
  for (let b = Math.ceil(minBPM / 10) * 10; b <= maxBPM; b += 10) yTicks.push(b);
  ...
  return (
    <div className={styles.wrap}>
      <p className={styles.hint}>BPM OVER TIME · hover to inspect · dashed = average</p>
      <svg
        viewBox={`0 0 ${W} ${H}`}
      ...
      >
        ...
        <g transform={`translate(${PAD.l},${PAD.t})`}>
          {/* Grid */}
          {yTicks.map((b) => (
            <line key={b} x1={0} y1={yS(b)} x2={IW} y2={yS(b)}
              stroke="#1e1e1e" strokeWidth={1} />
          ))}

          {/* Fill */}
          <polygon clipPath="url(#chartClip)" fill="url(#fillGrad)" points={fillPts} />

          {/* Curve */}
          <polyline clipPath="url(#chartClip)" points={pts}
            fill="none" stroke="#e8c96a" strokeWidth={2}
            strokeLinejoin="round" strokeLinecap="round" />

          {/* Average line */}
          <line x1={0} y1={yS(avgBPM)} x2={IW} y2={yS(avgBPM)}
            stroke="#e8c96a" strokeWidth={1} strokeDasharray="4 4" opacity={0.45} />

          {/* Playhead */}
          {playheadX != null && playheadX >= 0 && playheadX <= IW && (
            <line x1={playheadX} y1={0} x2={playheadX} y2={IH}
              stroke={`var(--danger)`} strokeWidth={1.5} opacity={0.75} />
          )}

          {/* Hover dot + tooltip */}
          {hovered && (
            <>
              <circle cx={xS(hovered.t)} cy={yS(hovered.bpm)} r={4} fill="#e8c96a" />
              <g transform={`translate(${tooltipX(hovered)},${Math.max(4, yS(hovered.bpm) - 36)})`}>
                <rect x={0} y={0} width={82} height={38} rx={3}
                  fill="#111" stroke="#e8c96a" strokeWidth={1} />
                <text x={8} y={14} fill="#888" fontSize={10} fontFamily="monospace">
                  {formatTime(hovered.t)}
                </text>
                <text x={8} y={30} fill="#f0f0f0" fontSize={13}
          ...
          {/* Y-axis labels */}
          {yTicks.filter((b) => b % 20 === 0).map((b) => (
            <text key={b} x={-8} y={yS(b) + 4} textAnchor="end"
              fill="#555" fontSize={10} fontFamily="monospace">{b}</text>
          ))}
